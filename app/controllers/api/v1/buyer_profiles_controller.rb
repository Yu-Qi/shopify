    ...
    class BuyerProfilesController < ApplicationController
      ...
      def show
        ...
        if profile
          ...
          render json: { error: '買家資料不存在' }, status: :not_found
        end
      end

      def create
        if current_buyer_profile
          render json: { error: '買家資料已存在' }, status: :unprocessable_entity
          return
        end
      ...
      def destroy
        ...
        profile.destroy!
        render json: { message: '買家資料已刪除' }, status: :ok
      end

      private

      def profile_params
        params.require(:buyer_profile).permit(:display_name)
      end
